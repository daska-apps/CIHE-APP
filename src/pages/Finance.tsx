    ...
    <div className="space-y-12 pb-20">
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div className="space-y-1">
          ...
          <h1 className="text-4xl md:text-5xl font-display font-black text-slate-800 tracking-tight">Finances & Fees</h1>
          <p className="text-slate-500 font-medium max-w-md">Manage tuition, HECS/FEE-HELP, and SA-HELP fees under Australian student loan regulations.</p>
        </div>

        <div className="flex gap-3">
          <button className="px-6 py-3 bg-slate-900 text-white rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-brand-indigo transition-all shadow-xl shadow-black/5">
            Pay Outstanding
          </button>
        </div>
      </header>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <motion.div
            ...
            className="lg:col-span-2 bg-white rounded-[3rem] p-10 border border-slate-200 shadow-sm flex flex-col md:flex-row items-center gap-10"
          >
              <div className="text-center md:text-left space-y-2">
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Outstanding Balance</p>
                  <p className="text-5xl font-display font-black text-slate-800">$320.00</p>
              ...
              <div className="h-px md:h-20 w-full md:w-px bg-slate-100" />
              <div className="flex-1 space-y-4">
                  <div className="flex justify-between items-center text-sm">
                      <span className="font-bold text-slate-500">Academic Census Date</span>
                      <span className="font-black text-rose-600">31 May 2026</span>
                  </div>
                  <div className="flex justify-between items-center text-sm">
                      <span className="font-bold text-slate-500">Government HELP Status</span>
                  ...
                  <div className="flex justify-between items-center text-sm">
                      <span className="font-bold text-slate-500">Invoiced Currency</span>
                      <span className="font-black text-slate-800">AUD (Australian Dollars)</span>
                  </div>
              </div>
          </motion.div>
                  ...
                  <table className="w-full text-left border-collapse">
                      <thead>
                          <tr className="bg-slate-50/50 text-[10px] font-black text-slate-400 uppercase tracking-widest">
                              <th className="px-8 py-5 border-b border-slate-100">Invoice ID</th>
                              <th className="px-8 py-5 border-b border-slate-100">Issue Date</th>
                      ...
                      <tbody className="divide-y divide-slate-50">
                          {INVOICES.map((inv, i) => (
                              <tr key={i} className="hover:bg-slate-50/50 transition-colors">
                                  <td className="px-8 py-5 font-mono text-[11px] font-black text-slate-400 uppercase">{inv.id}</td>
                                  <td className="px-8 py-5 text-xs font-bold text-slate-500">{inv.date}</td>
                                  <td className="px-8 py-5 text-sm font-bold text-slate-700">{inv.desc}</td>
                                  ...
                                  <td className="px-8 py-5 text-right font-display font-black text-slate-800">${inv.amount.toLocaleString()}</td>
                                  <td className="px-8 py-5 text-right">
                                      <button className="p-2 bg-slate-100 rounded-xl text-slate-400 hover:text-brand-indigo hover:bg-indigo-50 transition-all">
